# 🌱 !Hola mundo!  

## Sobre mí 
Soy de Ecuador y soy autodidacta, creo que la inteligencia artificial puede transformar el mundo para mejor... y quiero ser parte de eso  
Aunque estoy empezando, me encanta investigar, probar herramientas y escribir sobre lo que aprendo.  

## 🧩 ¿Qué estoy aprendiendo ahora?  
- Historia y ética de la IA  
- Diferencias entre Machine Learning y Deep Learning  
- Cómo usar herramientas como ChatGPT y sus competidores
- Primeros pasos con Python (¡hola, mundo!)  

## 📚 Recursos que me inspiran   
- Canales de YouTube: [Midudev], [Fazt], [munoncode], [MoureDev]   
- Podcasts o Videos: [Inteligencia Artificial], [Gustavo Entrala], [Nate Gentile]
- Empresas Tech: [OpenAI], [Anthropic], [Cursor], [Trae] 

## 💡 Proyectos pequeños (idea)  
1. **App para estudiantes de Preuniversitarios**  
   - Busca resolver el gran problema del método tradicional —su alto costo y exclusividad — al ofrecer una alternativa accesible y económica, incluso gratuita, que democratice el acceso a recursos educativos de calidad, guías personalizadas y una comunidad de apoyo, permitiendo a jóvenes de todos los contextos prepararse para la universidad sin sacrificar su economía, mientras desarrollan habilidades académicas, emocionales y sociales clave para su futuro..  
    

## 🎯 Metas próximas  
- Crear webs, apps y webapps integradas con IA para solucionar problemas reales de la comunidad
- Entender qué es un "prompt" y cómo optimizarlo para el dia a dia.
- Conectar con otros apasionados de la IA 🤝.  

## 📨 ¡Hablemos!  
¿Tienes consejos para alguien que empieza? ¿Quieres colaborar en algo? ¡Escríbeme!  


## 🛠️ Stack Tecnológico  
Utilicé las siguientes herramientas y tecnologías para desarrollar este proyecto:  

![Pila de tecnología](https://img.shields.io/badge/HTML5-%23E34F26.svg?style=for-the-badge&logo=html5&logoColor=white)
![Pila de tecnología](https://img.shields.io/badge/CSS3-%231572B6.svg?style=for-the-badge&logo=css3&logoColor=white)
![Pila de tecnología](https://img.shields.io/badge/JavaScript-F7DF1E.svg?style=for-the-badge&logo=javascript&logoColor=black)
![Pila de tecnología](https://img.shields.io/badge/Python-FFD43B.svg?style=for-the-badge&logo=python&logoColor=blue)
![Pila de tecnología](https://img.shields.io/badge/Pygame-660000?style=for-the-badge&logo=pygame&logoColor=white)
![Pila de tecnología](https://img.shields.io/badge/Canvas-%2320232a.svg?style=for-the-badge&logo=canvas.js&logoColor=%23fff)
![Pila de tecnología](https://img.shields.io/badge/VSCode-0078d7.svg?style=for-the-badge&logo=visual-studio-code&logoColor=white)

